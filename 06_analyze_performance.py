...
gpt_df = pd.read_excel("data/projects_chatgpt_flags_tagged.xlsx", index_col=0)
...
result_df.to_excel('output/performance test.xlsx')

pd.crosstab(df.bot_ph_flag, df.human_ph_flag).to_csv('output/disagreement xtab.csv')

flag_diffs = df.loc[(df.bot_ph_flag!=df.human_ph_flag)]
flag_diffs.to_excel('output/flag disagreements.xlsx', index=False)
pass